...
public class SList {
  ...
  public void squish() {
	...
	if (currentNode.next.item.equals((currentNode).item)) {
		SListNode Temp= currentNode.next.next;
  ...
  public void twin() {
    // Fill in your solution here.  (Ours is seven lines long.)
	  ...
	  if (head==null){
		  ...
		  int count=0;
		  while((currentNode.next!=null)&&(count<100)){
			count++;
			SListNode duplicate=new SListNode(currentNode.item);
			duplicate.next = currentNode.next;
			currentNode.next=duplicate;
/*			currentNode.next = new SListNode(currentNode.item);
			currentNode.next.next=duplicate.next;*/

			int temp1=(int)duplicate.item;
			int temp2=(int)currentNode.item;
			int temp3=(int)(duplicate.next.item);
